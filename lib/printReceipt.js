...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .store { font-size: ${fs + 4}px; font-weight: bold; letter-spacing: 0.5px; text-align: center; }
    .branch-name { font-size: ${fs + 1}px; font-weight: bold; text-align: center; }
...
<body>
  ...
  <p class="store">POS FROZEN FOOD</p>
  ...
  <script>
    window.onload = function () {
      window.focus();
      window.print();
      // tutup tab setelah print dialog ditutup (tidak semua browser mendukung)
  ...
  const winW = size === "58mm" ? 320 : 420;
  const win = window.open("", "_blank", `width=${winW},height=750,scrollbars=yes`);
  if (!win) {
    alert("Pop-up diblokir. Izinkan pop-up di browser untuk mencetak struk.");
    return;
  }
